...
- `src/lib/habits.ts` — the core: `isScheduledOn()` (which habits appear on a given day; returns false for `archived`), `periodKeyFor()`, `toggleCompletion()`, `dayStatus()` (aggregates a day's completion ratio), plus the stats layer: `scheduledOccurrences()`, `habitStats()` (current/best streak + completion rate; the in-progress current period being unticked does not break the current streak) and `recentHistory()` (heatmap cells). A habit is never scheduled before its `createdAt`.
- `src/lib/theme.ts` — the **monochrome theme**: `useTheme()` derives a black/white palette from the OS color scheme **and** the user's `settings.theme` override (`system`/`light`/`dark`), so it reads from `useStore()` — only callable inside `StoreProvider`. Exposes the `Theme` type and `font` (Public Sans names). Components build styles via a local `makeStyles(theme)` memoized on the theme. `ratioColor(ratio, colorblind?)` maps a completion ratio to the red→orange→yellow→green scale (or a blue→yellow colourblind-safe scale) used by calendar cells — kept colourful on purpose, independent of the monochrome chrome. Per-habit colours (`HABIT_COLORS`) are user data and also stay colourful.

User **settings** (`theme`, `weekStart`, `colorblind`) live in the same persisted blob and are edited via `updateSettings()`; habits can be reordered (`moveHabit`), paused (`setArchived`), and removed (with an in-modal confirm, since RN-web has no `Alert`).

`src/components/` is presentation only and defers all date/scheduling/stats math to `src/lib`: `CalendarScreen` (+ future-day preview dots, week-start aware), `HabitsScreen` (cards with streaks, all-done celebration), `HabitFormModal`, `HabitDetailModal` (stats + `Heatmap` + manage actions), `SettingsModal`, and the reusable `AnimatedCheck`. Haptics go through `src/lib/haptics.ts` (no-op on web).
